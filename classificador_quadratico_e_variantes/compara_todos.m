...
%D=load('sensor_readings_24.data');
%D=load('wine2.dat');
%D=load('coluna2.dat');
%D=load('wall2.dat');
%D=load('wall4.dat');
...
maxD=max(D(:,1:end-1));
minD=min(D(:,1:end-1));
%D(:,1:end-1)=(D(:,1:end-1)-minD)./(maxD-minD);  % Normaliza entre 0 e 1
%D(:,1:end-1)=2*((D(:,1:end-1)-minD)./(maxD-minD))-1;  % Normaliza entre -1 e 1

%%%%%%%%%% APLICA PCA
##classe=D(:,end);
##V=princomp(D(:,1:end-1));
##q=12; Vq=V(:,1:q); Q=Vq';
##aux = Q*D(:,1:end-1)';
##D=[aux' classe];
...
tic; [STATS_3 TX_OK3 X3 m3 S3 posto3]=variante3(D,Nr,Ptrain,0.150); Tempo3=toc; % Regularization method 2 (Friedman)
tic; [STATS_4 TX_OK4 X4 m4 S4 posto4]=variante4(D,Nr,Ptrain); Tempo4=toc;     % Naive Bayes Local (Based on quadratico)
tic; [STATS_5 TX_OK5 W]=linearMQ(D,Nr,Ptrain); Tempo5=toc;     % Naive Bayes Local (Based on quadratico)
...
%set(gca (), "xtick", [1 2 3 4 5 6 7 8 9], "xticklabel", {"Maha1","Maha2", "Maha3","Naive1","Naive2","Naive3","EUC","MAN","MQ"})
